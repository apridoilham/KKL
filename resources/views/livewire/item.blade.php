<div class="container-fluid px-4 md:px-6 py-6">
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-800">Manajemen Barang</h1>
            <p class="mt-1 text-slate-600">
                @if($items->total() > 0)
                    Daftar lengkap semua barang. Anda saat ini memiliki <span class="font-bold text-indigo-600">{{ $items->total() }}</span> jenis barang.
                @else
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse ($items as $index => $item)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 text-center"><div class="flex items-center justify-center space-x-2"><button wire:click="edit({{ $item->id }})" class="p-2 rounded-full text-blue-600 hover:bg-blue-100" title="Ubah Data"><i class="fas fa-pen fa-sm"></i></button>@if(auth()->user()->role == 'admin')<button wire:click="delete({{ $item->id }})" wire:confirm="Anda yakin ingin menghapus barang ini?" class="p-2 rounded-full text-red-600 hover:bg-red-100" title="Hapus Data"><i class="fas fa-trash fa-sm"></i></button>@endif</div></td>
                        </tr>
                    @empty
                        <tr><td colspan="8" class="text-center py-16 px-4"><svg class="mx-auto h-12 w-12 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 13h6m-3-3v6m-9 1V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg><h3 class="mt-2 text-lg font-semibold text-slate-800">Barang Tidak Ditemukan</h3><p class="mt-1 text-sm text-slate-500">Mulai dengan menambahkan barang pertama Anda.</p><div class="mt-6"><button wire:click="create" class="inline-flex items-center px-4 py-2.5 border text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700"><i class="fas fa-plus mr-2"></i>Tambah Barang Pertama</button></div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
